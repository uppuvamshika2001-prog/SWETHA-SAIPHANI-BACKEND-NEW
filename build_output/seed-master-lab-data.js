...
async function main() {
    ...
    for (const testData of tests) {
        ...
        const existing = await prisma.labTest.findFirst({ where: { code: testData.code } });
        let test;
        if (existing) {
        ...
        for (const catData of testData.categories) {
            ...
            for (const pData of catData.parameters) {
                await prisma.labTestParameter.upsert({
                    where: {
                        id: (await prisma.labTestParameter.findFirst({
...
//# sourceMappingURL=seed-master-lab-data.js.map