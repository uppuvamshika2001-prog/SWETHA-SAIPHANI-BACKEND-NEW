...
import { AppError, ValidationError, UnauthorizedError, ForbiddenError, NotFoundError, ConflictError, isOperationalError, } from '../utils/AppError.js';
...
function handlePrismaError(err) {
    switch (err.code) {
        ...
        case 'P2011':
            ...
            const nullTarget = err.meta?.constraint?.join(', ') || 'required field';
            return {
 ...
 */
function sendErrorResponse(res, statusCode, errorCode, message, requestId, details, stack) {
    const response = {
        status: 'error',
        message,
        code: statusCode,
...
export function errorHandler(err, req, res, _next) {
    ...
    if (err instanceof AppError) {
        sendErrorResponse(res, err.statusCode, err.code, err.message, requestId, err.details, err.stack);
        return;
    }
    // Handle Zod validation errors
    ...
    if (err instanceof Prisma.PrismaClientValidationError) {
        sendErrorResponse(res, 400, 'DATABASE_VALIDATION_ERROR', 'Invalid data provided', requestId, undefined, err.stack);
        return;
    }
    // Handle JWT errors
    ...
    if (err instanceof SyntaxError && 'body' in err) {
        sendErrorResponse(res, 400, 'INVALID_JSON', 'Invalid JSON in request body', requestId, undefined, err.stack);
        return;
    }
    // Handle TypeError (usually accessing property on undefined)
    if (err instanceof TypeError) {
        logger.error({ err }, 'TypeError - possible null/undefined access');
        sendErrorResponse(res, 500, 'INTERNAL_ERROR', config.isProduction ? 'Internal server error' : err.message, requestId, undefined, err.stack);
        return;
    }
    ...
    sendErrorResponse(res, 500, 'INTERNAL_ERROR', config.isProduction ? 'Internal server error' : err.message, requestId, undefined, config.isProduction ? undefined : err.stack);
}
/**
...
export function notFoundHandler(req, res) {
    const requestId = getRequestId(req);
    sendErrorResponse(res, 404, 'ROUTE_NOT_FOUND', `Route ${req.method} ${req.originalUrl} not found`, requestId);
}
