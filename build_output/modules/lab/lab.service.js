...
export class LabService {
    async createOrder(orderedByUserId, userRole, input) {
        ...
        const order = await prisma.labTestOrder.create({
            data: {
                ...
                walkInName: isWalkIn ? (input.walkInName || null) : null,
                walkInPhone: isWalkIn ? (input.walkInPhone || null) : null,
        ...
        try {
            ...
            if (statusQuery) {
                ...
                if (upperStatus === 'PENDING' || upperStatus === 'PAYMENT_PENDING') {
                    ...
                    where.status = Object.values(LabTestStatus).includes(upperStatus)
                        ? upperStatus
    ...
    async getMyOrders(userId, query) {
        try {
            ...
            const skip = (Number(page) - 1) * Number(limit);
            const where = { orderedById: staff.id };
            if (statusQuery) {
                const upperStatus = statusQuery.toString().toUpperCase();
                // Map PENDING to PAYMENT_PENDING for backward compatibility
                if (upperStatus === 'PENDING' || upperStatus === 'PAYMENT_PENDING') {
    ...
    async getOrderParameters(orderId) {
        try {
            ...
            const needsFallback = order && (!order.test ||
                !order.test.categories ||
                order.test.categories.length === 0 ||
                order.test.categories.every((c) => !c.parameters || c.parameters.length === 0));
            if (needsFallback) {
                const resolvedTest = await prisma.labTest.findFirst({
                    where: {
                        isActive: true,
            ...
            const patientName = order.patient
                ...
                : (order.walkInName || 'Walk-in Patient');
            const patientGender = order.patient ? order.patient.gender : null;
            ...
            const getRange = (p) => {
                ...
                if (!range || typeof range !== 'object') {
                    ...
                    if (patientGender === 'MALE' && p.referenceRangeMale)
                        finalRange = p.referenceRangeMale;
                    else if (patientGender === 'FEMALE' && p.referenceRangeFemale)
                        finalRange = p.referenceRangeFemale;
                    else
                        finalRange = p.normalRange || (p.normalMin !== null && p.normalMax !== null ? `${p.normalMin} - ${p.normalMax}` : '');
                }
                else {
                    // New JSON logic
                    if (patientGender === 'MALE' && range.male)
                        finalRange = range.male;
                    else if (patientGender === 'FEMALE' && range.female)
                        finalRange = range.female;
                    ...
                    if (!finalRange && range.adults && patientAgeNum >= 18)
                        finalRange = range.adults;
                    if (!finalRange && range.newborn && patientAgeNum < 1)
                        finalRange = range.newborn;
                    if (!finalRange)
                        finalRange = range.default || range.general || Object.values(range)[0] || '';
                }
                if (typeof finalRange !== 'string')
                    finalRange = String(finalRange);
            ...
            const fallbackParams = (parameters || []).map((p) => ({
                id: p.id,
                name: p.parameterName,
                unit: p.unit || '',
                inputType: p.inputType || 'number',
    ...
    async submitResult(technicianUserId, input) {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (input.result.parameters && input.result.parameters.length > 0) {
                const catalogResults = input.result.parameters
                    .filter((p) => p.parameterId && !p.isManual)
                    .map((p) => ({
                    orderId: input.orderId,
                ...
                // Log for visibility.
                const manualCount = input.result.parameters.filter((p) => p.isManual || !p.parameterId).length;
                if (manualCount > 0) {
            ...
            await tx.labTestOrder.update({
                ...
                data: {
                    status: 'COMPLETED',
                    isReportVisibleToPatient: input.isReportVisibleToPatient !== undefined ? input.isReportVisibleToPatient : true
                },
            });
            return labResult;
        });
    ...
    formatOrder(order) {
        return {
            ...
            status: order.status,
            notes: order.notes,
            patient: order.patient || { firstName: order.walkInName || 'Walk-in', lastName: '' },
            ...
            walkInPhone: order.walkInPhone || null,
            ...
            if (normalizedCode !== test.code) {
                ...
                if (existingTest) {
                    ...
                    if (namesMatch) {
                        ...
                        await prisma.labTestParameter.deleteMany({ where: { testId: id } });
                        await prisma.labTestCategory.deleteMany({ where: { testId: id } });
    ...
    async generateReportPDF(orderId) {
        const order = (await prisma.labTestOrder.findUnique({
            where: { id: orderId },
            include: {
                patient: true,
        ...
        const resultBlob = order.result.result;
        const processedParameters = (resultBlob.parameters || []).map((p) => {
            // Find corresponding catalog parameter in results join
            const catalogMatch = order.labResults?.find((lr) => lr.parameterId === p.parameterId);
            ...
            if ((!referenceRange || referenceRange === '-') && catalogMatch?.parameter) {
                ...
                if (!referenceRange && param.referenceRange) {
                    ...
                    if (patientGender === 'MALE' && range.male)
                        ...
                        referenceRange = range.female;
                    else
                        referenceRange = range.default || range.general || '';
                }
